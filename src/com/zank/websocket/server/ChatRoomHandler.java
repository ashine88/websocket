...
import com.zank.websocket.common.entity.CTResponseBody;
...
public class ChatRoomHandler extends SimpleChannelInboundHandler<CTRequest>{
	...
	private void handleWebsocketRequest(CTWebScoketRequest req){
		JsonObject json = new Gson().fromJson(req.getBody(), JsonObject.class);
		String method = json.get("method").getAsString();
		if("join".equals(method)){
			join(json.get("body").getAsJsonObject());
		}else if("sendMessage".equals(method)){
			sendMessage(json.get("body").getAsJsonObject());
		}else if("exit".equals(method)){
			exit(json.get("body").getAsJsonObject());
		}
	}
	private void create(CTHttpRequest req){
		JsonObject data = new Gson().fromJson(req.getBody(), JsonObject.class);
	...
	private void sendMessage(JsonObject data){
		...
		if(null !=  room){
			if(room.isExists(from)){
				System.out.println(String.format("用户【%d】存在于聊天室【%d】", from, roomId));
